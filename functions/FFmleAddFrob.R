# FFmleAddFrob: computes iPCA covariance estimates via Flip-Flop MLE
# with Additive Frobenius norm penalty
#
# inputs:
...
FFmleAddFrob <- function(dat, lamDs, lamS,
  ...
  while (ind > thr & iter < maxit) {
    ...
    inD <- mclapply(X = inD,
                    FUN = function(X) {return(1/2 * (X + t(X)))}, mc.cores = n_cores) # to ensure symmetry
  ...
  if (ind > thr) {
    message('Warning: Additive Frobenius Sigma estimate did not converge.')
  }else {
    cat(paste0('Additive Frobenius Estimate converged after ', iter-1, ' iterations. \n'))
  }
